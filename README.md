  ...
  pr-walkthrough:
    ...
    steps:
      ...
      - uses: freewho99/opensoyce-pr-walkthrough-action@v1
        with:
          opensoyce-version: '0.2.2' # pin to a real published version; bump on your own schedule
...
| `opensoyce-version` | yes | The exact published `opensoyce` npm version to run, e.g. `0.2.2`. No default — pin deliberately |
